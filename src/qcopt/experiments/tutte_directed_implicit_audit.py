"""Realistic-resolution topology and VJP audit for directed positive Tutte rows."""

from __future__ import annotations

import argparse
import json
import time
from pathlib import Path

import numpy as np
import torch

from qcopt.forward.tutte_directed_implicit import (
    DirectedTutteSystem,
    directed_tutte_embedding_torch_implicit,
)
from qcopt.injectivity import audit_injectivity
from qcopt.mesh import structured_rectangle


def _target_boundary(mesh, loop: np.ndarray) -> np.ndarray:
    points = mesh.vertices[loop]
    linear = np.array([[1.08, 0.16], [0.04, 0.93]], dtype=np.float64)
    return np.ascontiguousarray(points @ linear.T)


def _logits(system: DirectedTutteSystem, rng: np.random.Generator, spread: float) -> np.ndarray:
    values = np.zeros((system.n_rows, system.max_degree), dtype=np.float64)
    values[system.valid_mask] = spread * rng.normal(size=int(np.count_nonzero(system.valid_mask)))
    return values


def run(output_dir: Path, n: int = 256, seed: int = 20260919) -> dict:
    output_dir.mkdir(parents=True, exist_ok=True)
    torch.set_default_dtype(torch.float64)
    mesh = structured_rectangle(n, n)
    system = DirectedTutteSystem.from_mesh(mesh)
    boundary = _target_boundary(mesh, mesh.boundary_loops[0])
    rng = np.random.default_rng(seed)
    records = []
    for spread in (1.0, 3.0):
        logits_np = _logits(system, rng, spread)
        boundary_t = torch.as_tensor(boundary, dtype=torch.float64)
        logits_t = torch.as_tensor(logits_np, dtype=torch.float64).requires_grad_(True)
        started = time.perf_counter()
        mapped_t = directed_tutte_embedding_torch_implicit(mesh, boundary_t, logits_t, system)
        loss = torch.mean((mapped_t[system.interior] - torch.as_tensor(mesh.vertices[system.interior], dtype=torch.float64)) ** 2)
        loss.backward()
        elapsed = time.perf_counter() - started
        mapped = mapped_t.detach().cpu().numpy()
        # The affine target is a convex parallelogram rather than an axis-
        # aligned rectangle, so use the disk/convex-boundary audit.
        report = audit_injectivity(mesh, mapped, rectangle=False)
        records.append(
            {
                "logit_spread": spread,
                "elapsed_seconds": float(elapsed),
                "loss": float(loss.detach().cpu()),
                "finite_output": bool(torch.isfinite(mapped_t).all()),
                "finite_gradient": bool(torch.isfinite(logits_t.grad).all()),
                "gradient_l2": float(torch.linalg.vector_norm(logits_t.grad).detach().cpu()),
                "flipped_faces": int(len(report.flipped_faces)),
                "minimum_signed_area": float(report.minimum_signed_area_ratio),
                "injectivity_certified": bool(report.certified),
                "boundary_orientation_ok": bool(report.boundary_orientation_ok),
                "boundary_intersections": int(len(report.boundary_intersections)),
                "bad_branch_vertices": int(len(report.bad_branch_vertices)),
            }
        )
    result = {
        "grid": f"{n}x{n} cells",
        "vertices": int(mesh.n_vertices),
        "faces": int(mesh.n_faces),
        "interior_rows": int(system.n_rows),
        "max_degree": int(system.max_degree),
        "records": records,
        "scope": "directed positive-row Tutte decoder with sparse implicit boundary/logit VJP",
        "limitation": "numerical directed-row evidence; no general nonsymmetric Tutte theorem or arbitrary-mu expressivity claim",
    }
    (output_dir / "tutte_directed_implicit_audit.json").write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    return result


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--n", type=int, default=256)
    parser.add_argument("--seed", type=int, default=20260919)
    args = parser.parse_args()
    print(json.dumps(run(args.output_dir, args.n, args.seed), indent=2))


if __name__ == "__main__":
    main()
