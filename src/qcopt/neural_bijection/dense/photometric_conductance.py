...
class PhotometricSpectralTutteLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        if (side < 3 or fit_side < 2 or ridge <= 0 or coefficient_bound <= 0
                or not frequencies or any(k < 1 or k >= (side-1)//2 for k in frequencies)):
    ...
    def prepare(self, *, device: torch.device | str) -> dict[str, float | int]:
        ...
        derivative = tuple(15*sigmoid*(1-sigmoid)*part for part in bases)
        conductances = tuple(torch.full_like(part,constant) for part in bases)
        source = self.solver._source[None].expand(count,-1,-1,-1)
        ...
        self._response_query = table.interpolate(full.float().reshape(count,-1,2))
