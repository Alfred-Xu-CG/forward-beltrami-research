...
affine on every triangle. On (T), write
\[
p_T=\nabla u_h|_T,qquad r_T=\nabla v_h|_T,qquad
A_T=A_T^T\succ0,quad\det A_T=1.
\]
...
\(f_{\bar z}=\mu_Tf_z\) for (f_h=u_h+iv_h), and positivity of
...
From (DC-1) and (A_TJA_T=J) for determinant-one symmetric tensors,
\[
...
\[
g(0)=0,quad g(1/2)=0.2,quad g(1)=1,
\]
