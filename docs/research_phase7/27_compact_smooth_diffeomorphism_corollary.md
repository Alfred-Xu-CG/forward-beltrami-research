...
行列式严格正来自每个 $F_t$ 都是保向微分同胚，紧致性使正的最小值存在。把这些**依赖 $F$** 的常数代入 F1/F2 的**guard-free理想实数**定理，选任何 $0\le\beta<m_F/4$，则对该 $F$ 存在一个有限的粗 seed 侧长与轮数，此后每级 F1 一轮或 F2 一周期，使最终固定规则三角网 P1 输出恰为 $I_hF$，并满足 $\|I_hF-F\|_\infty\le K_Fh^2$；各最终分辨率的 decoder 前向工作/显存渐近 $O(V_h)$，无全局大线性系统。输出为单张原网格 P1 同胚，而不是若干连续 P1 因子复合后采样。固定dtype实现的正分母guard在极小$m_F$时还需[F1](23_uniform_isotopy_multilevel_approximation.md)或[F2](26_f2_uniform_isotopy_approximation.md)所列附加余量条件；浮点实际可达性不由本推论自动给出。
...
“绝大部分 homeomorphism”若没有指定测度、拓扑或正则类没有数学意义。本文证明的是整个明确的无限维群 $\operatorname{Diff}^{\infty}_c(\Omega)$ 的**逐元素**逼近（解码架构常数依目标而异），不是关于所有边界固定 homeomorphism 的测度一声明，也没有证明用单一固定 seed 深度对这个无界群作统一近似。把结论升格为一般连续平面同胚的$C^0$稠密性还需**相同边界条件**下的内部紧支撑光滑近似；该步骤不由这里的disk isotopy自动给出，已在后续[定性稠密性证明](30_all_boundary_homeomorphisms_c0_density.md)中单独完成。
