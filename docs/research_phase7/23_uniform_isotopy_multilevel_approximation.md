...
一个待验证的实现方向是按分支定义非零分母：允许损失$u>0$时用$\max(u,\ell)$，$u=0$时用常数1；不利损失$\ell=0$时令该面尺度为1。这样前向公式无需正guard且不会计算$0/0$，但在$u=0$及活动面切换处的自动微分行为、编译后数值拓扑与大网格训练稳定性**尚未验证**，不能作为本轮已实现成果。
