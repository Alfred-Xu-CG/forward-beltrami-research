# 4097²完整图像层CPU暂存：约5.86GB显存可训练，但约5.7倍慢、占用大量主机内存
...
CPU暂存使本次完整4097²训练的allocated峰值低于6GB，但20步中位训练时间是直接方案的约5.69倍。第一次VJP用时18.08s，较热态4.26s明显更慢，不能把一次冷态混入中位。运行中用`ps`观察到该单训练进程主机RSS约**31.3百万KiB（约29.9GiB）**；这是一个时间点的进程常驻量，**不是峰值证明**，也不全由saved tensor造成，仍足以说明显存降低需要大量主机内存/传输。主机当时有可用内存约944GiB，所以没有与他人争抢内存的迹象；不能据此假设普通工作站也能承受。
...
阶段结论须把“可在小GPU容纳”与“快、总体低内存”分开：当前全量CPU offload提供了前者的一个实验性路径，却明显损害后者。下一步若把它用作实际训练方案，应测试**选择性**保存/重算与块式局部VJP，同时计入pinned CPU内存峰值、PCIe吞吐、batch扩展及所有训练步的拓扑接受率；不能只展示CUDA allocated一列。
