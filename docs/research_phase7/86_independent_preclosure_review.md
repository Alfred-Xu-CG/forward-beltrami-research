...
| 全体边界固定同胚的\(C^0\)稠密 | [相对边界光滑化与缩放共轭](30_all_boundary_homeomorphisms_c0_density.md)、[紧支撑光滑同伦](27_compact_smooth_diffeomorphism_corollary.md)、[Hatcher原文](https://ems.press/content/serial-article-files/50953) | 只能对**guard-free理想实数算子的所有架构规模之并、逐目标**宣称；不含统一速度、固定网络深度、固定dtype实现可达性或图像逆推。该命题还取`minimum_jacobian=None`，即无统一\(0.05h^2\)绝对floor；实验可设0.05及正guard，不能反过来证明全群密度。 |
