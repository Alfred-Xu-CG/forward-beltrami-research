...
| F1/F2对统一光滑同伦类的定量逼近 | [F1](23_uniform_isotopy_multilevel_approximation.md)、[F2](26_f2_uniform_isotopy_approximation.md)的面积、边长、时间速度界 | 给定统一\(m,L,K,M\)及边界恒等的\(C^{1,1}\)同伦，可选择**类别共同**的足够细seed和有限seed周期，使**guard-free理想实数算子**每次二分后一个F1颜色周期或F2交错周期到达目标顶点样本，误差\(O(h^2)\)，工作\(O(V)\)。这是teacher latent的存在性，不是训练好的固定小CNN对该类的统一逼近。实际固定dtype的正分母guard若超过面积允许量，可能截断一步到位；文档已补充guard不活动的定量充分条件，输出证书只保拓扑、不能补回可达性。 |
| 全体边界固定同胚的\(C^0\)稠密 | [相对边界光滑化与缩放共轭](30_all_boundary_homeomorphisms_c0_density.md)、[紧支撑光滑同伦](27_compact_smooth_diffeomorphism_corollary.md)、[Hatcher原文](https://ems.press/content/serial-article-files/52184) | 只能对**guard-free理想实数算子的所有架构规模之并、逐目标**宣称；不含统一速度、固定网络深度、固定dtype实现可达性或图像逆推。该命题还取`minimum_jacobian=None`。独立构造的中心圆盘光滑压缩给出：若每面\(J_T\ge\beta>0\)，则对某目标有与网格尺度无关的正\(C^0\)误差下界；所以精确实数0.05 floor版本的全群密度不只是未证，而是错误。 |
| 低维latent可精确表示固定4097²网格的“绝大部分”P1同胚 | [固定网格维数论证](30_all_boundary_homeomorphisms_c0_density.md) | 若解码器权重固定且对\(m\)维latent局部Lipschitz，\(m<2(4095)^2\)，其输出在合法P1顶点表开集中的全维Lebesgue测度为零，故**不能**按体积测度精确覆盖几乎所有配置。只针对固定网格的精确覆盖，不否定任务分布低维性或有容差近似。 |
