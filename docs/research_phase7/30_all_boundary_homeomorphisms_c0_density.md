...
## 5. 固定正面积底线为什么确实阻止全群稠密

上述命题选\(\beta=0\)并非纯粹为了证明方便。对任意固定\(0<\beta<1\)，要求每张原面\(J_T\ge\beta\)的P1同胚集合**不可能**在所有边界固定同胚中\(C^0\)稠密。给一个光滑、紧支撑的显式反例：取中心\(c=(1/2,1/2)\)、\(r_0=1/8\)，选光滑径向截断\(\chi(r)\)，在\(r\le r_0\)等于1，在\(r\ge1/4\)等于0。令\(F_t\)为光滑向量场
\[
\dot x=-\lambda\chi(\|x-c\|)(x-c),\qquad \lambda=\log(1/\delta),\quad 0<\delta<\sqrt\beta
\]
的时间\(t\)流。它在外边界邻域恒等，处处为保向光滑微分同胚；对中心闭圆盘\(A=\overline{B(c,r_0)}\)，轨迹始终留在\(\chi=1\)区，故\(F_1(A)=\overline{B(c,\delta r_0)}\)。

若某张固定原网格P1同胚\(f\)满足每面\(J_T\ge\beta\)且\(\|f-F_1\|_\infty\le\varepsilon\)，单射映射的面积公式给
\[
\operatorname{area}(f(A))=\int_AJ_f\,dx\ge\beta\pi r_0^2.
\]
另一方面，逐点一致误差使\(f(A)\subseteq\overline{B(c,\delta r_0+\varepsilon)}\)，故其面积至多\(\pi(\delta r_0+\varepsilon)^2\)。于是**与网格尺度无关**地
\[
\varepsilon\ge(\sqrt\beta-\delta)r_0>0.
\]
例如数学上的\(\beta=0.05,\delta=0.1,r_0=1/8\)给误差下界约0.01545。该反例已是光滑且内部紧支撑，表明固定正floor妨碍的不是Hatcher光滑化步骤，而是面积压缩本身。它不声称现有浮点代码真的对所有输出保住精确\(0.05\)底线：代码另有舍入与最终**仅正向**的证书；这里否定的是精确实数\(J_T\ge\beta\)受限架构的全群密度。
...
## 7. 固定网格上低维latent的精确覆盖限制

固定一个\(N\times N\)控制网格和恒等边界，内点二维坐标合起来构成\(d=2(N-2)^2\)维实向量。所有原面严格正向是一组有限的严格连续不等式，因此合法顶点表集合\(\mathcal H_N\subset\mathbb R^d\)是非空**开集**。若固定网络权重后，解码器\(D:\mathbb R^m\to\mathcal H_N\)对latent局部Lipschitz且\(m<d\)，则\(D(\mathbb R^m)\)的\(d\)维Lebesgue测度为0：把参数域写成可数个有界块，每块的Lipschitz像的Hausdorff维数至多\(m\)。故它不能**精确表示**相对于\(\mathcal H_N\)体积测度“几乎所有”的固定网格P1同胚，也不可能含有开子集。4097²时\(d=2(4095)^2=33{,}538{,}050\)。

这个维数限制不否定低维latent对**特定任务分布**有用：真实形变可能集中在低维或近低维集合；也不排除一个目标依赖架构族在\(C^0\)意义下稠密、或固定低维latent在给定容差下很好地覆盖某个有限数据集。它只排除“固定、局部Lipschitz、\(m<d\)的解码器精确覆盖全维几乎每个P1配置”这一过强解释。若同时训练解码器权重，训练后作推理时这些权重仍固定，不能把模型参数偷偷算成每张图像的latent维数。
