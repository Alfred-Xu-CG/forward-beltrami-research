...
As an **oracle diagnostic only**, we trained the *original* 1006-parameter local encoder on target query-map MSE instead of image MSE, still with the same 32/8 split and 1000 steps. Target maps therefore entered the training loss, so this is **not** image-to-latent evidence. Held-out query-map RMSE was 0.002027, essentially the image-only encoder's 0.002022; the largest projected 32-cycle x/y amplitudes were only about \(1.1\times10^{-8}\) and \(1.1\times10^{-7}\). The held-out face-Beltrami RMSE was 0.13930. Since a direct per-instance A2+ latent fit did recover amplitudes around 0.0018–0.0020, this supervised negative result implicates the current amortized encoder architecture or its optimization, not just ambiguity of the image objective. It does not prove an impossibility for all encoders. See `raw_results/a2_high32_oracle_encoder_cpu1000_{train_summary,face_eval}.json` and its checkpoint.

The larger 3342-parameter contextual encoder received the same **target-map supervision** in another diagnostic run. Its held-out query-map RMSE was 0.001963, but every 32-cycle x/y projection remained below \(1.8\times10^{-8}\); face-Beltrami RMSE was 0.13949. Thus added pooled context alone did not restore the missing fine mode even with direct map supervision. The corresponding checkpoint and `raw_results/a2_high32_context_oracle_encoder_cpu1000_{train_summary,face_eval}.json` distinguish this result from genuine image-only training.
