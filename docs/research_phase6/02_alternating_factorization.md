...
Before the empirical count, a deliberately loose **verifiable upper bound** makes the representation cost explicit. Let the first and second factors have $T_1$ and $T_2$ source triangles. The image of one first-factor triangle is a triangle; its intersection with one second-factor source triangle is convex with at most six vertices. Any nonempty such intersection can be triangulated into at most four affine subtriangles. Hence the exact composition has an overlay triangulation with no more than $4T_1T_2$ affine triangles, even without locality assumptions. For two 257² factors this is $4(131072)^2=68,719,476,736$, wildly larger than the measured row-strip lower bounds; it is a worst-case mathematical bound, **not** a prediction that this implementation allocates tens of billions of pieces. The implemented compact query representation stores two original factors and dynamically locates each queried point, so its memory is governed by the two grids and query batch instead of explicitly realizing this bound.
...
### Two full fine-grid factors: expressivity is not amortized inference

The composition implementation also permits equal 257² factor grids. Its theorem is unchanged: each convex-subdivision P1 factor is a homeomorphism, so their *exact* composition is a PL homeomorphism; there is still **no claim** that reinterpolating its values on the original 257² triangles would preserve that property. A 17²+17² random-latent test found a dyadic source-edge midpoint whose composed image is noncollinear with its mapped endpoints, proving the single A2+ straight-edge constraint does not generally survive composition. This family has 175,772 direct latent scalars, about twice the single-factor count. The encoder used below has 2,012 trainable parameters.

For the deterministic high32 coordinate-supervised oracle, two 257² factors fitted the original 66,049 target vertices to coordinate-component RMSE 0.000269 after 1,000 Adam steps and 0.000191 after another 2,000 smaller-rate steps. The saved single A2+ comparison was 0.000444/0.000356, and 17²+257² was 0.000769/0.000601. Independent chain-Jacobian evaluation at all 131,072 original source-face centroids gave complex Beltrami RMSE 0.10582/0.10274 at the two checkpoints, versus 0.12202 for the 3,000-step single A2+ checkpoint. The 32-cycle Fourier projection of the composed vertex displacement reached (0.00233, 0.00232) after 3,000 steps, versus target (0.00250, 0.00250); this direct-fit oracle therefore actually uses fine detail. Its two minimum normalized factor areas remained 0.3135 and 0.4269; sampled chain determinant minimum was 0.3229. These are not amortized or image-only results, and centroid samples are not an integral over the full composition overlay.

When *the same two-fine decoder* was trained with separate small image encoders on 32 training and eight disjoint held-out images, with 512² queries, batch two and 1,000 image-only Adam steps, held-out image MSE was 0.000505 and query-map RMSE 0.001930. Source-centroid Beltrami RMSE was 0.14998, maximum sampled \(|\mu|=0.8344\), and the minimum area of its first factor fell to 0.0993. Most decisively, all eight predicted 32-cycle projection amplitudes were near \(10^{-6}\), despite true values about \(10^{-3}\). This is worse local geometry than the coordinate-supervised oracle and does not establish a successful neural predictor of fine detail. The measured median full training forward/VJP was 51.0/69.5 ms on Element CPU, with sampled process RSS 990 MB and 123.5 s training wall.

To test image-loss sensitivity without consulting target maps, we added \(\lambda\) times the mean squared one-pixel horizontal/vertical finite differences of the warped-minus-fixed image residual. At \(\lambda=10\), held-out image/query-map MSE and RMSE were 0.000508/0.001899; centroid Beltrami RMSE decreased to 0.14085 and maximum sampled \(|\mu|\) to 0.5005, but the fine projection still did not track its signed target amplitude. At \(\lambda=100\), the image/query-map errors worsened to 0.000586/0.001976, centroid Beltrami RMSE was 0.13925, and the fine projection was nearly constant around \(10^{-6}\) across targets of both signs. The stronger derivative objective mainly regularized observed local distortion; neither tested weight solved amortized high-frequency inference. Raw train summaries, independent face evaluations, and checkpoints use `twofine257_high32_*` names.

For a separate identifiability diagnostic, we optimized all 175,772 latents **directly for one held-out pair** using image MSE only, from the identity, with no encoder and no true map in the loss. After 1,000 steps the image MSE was just \(7.80\times10^{-7}\), yet query-map RMSE was 0.002561, source-centroid Beltrami RMSE 0.2815, maximum sampled \(|\mu|=0.9383\), and the two minimum factor areas were only 0.0400/0.0577. The predicted 32-cycle amplitude was (0.00120, 0.00141), compared with the true 0.002096 in each coordinate. Thus the image pair does contain *some* usable high-frequency signal, but near-perfect single-channel image matching here does not identify the correct map or good QC geometry. This does not prove a fundamental impossibility for richer image features or a learned prior; it shows that more decoder freedom and per-instance fitting alone can aggravate the ambiguity. The one-pair checkpoint and metrics are `twofine257_high32_pair1_image_latent1000.*`.

### Exact composition scaling at fixed image resolution

On Element CPU, float32, four threads, batch one, 17² first factor and 512² fixed image queries, after two warmups and three repeats, exact composition plus image warp/loss and its first-order VJP measured:

| Fine factor controls | Fine factor triangles | Latent scalars | Median forward | Median VJP | Sampled process RSS | Minimum factor area ratios |
|---:|---:|---:|---:|---:|---:|---:|
| 257² | 131,072 | 88,252 | 18.8 ms | 21.8 ms | 611 MB | 0.748 / 0.562 |
| 513² | 524,288 | 350,908 | 21.6 ms | 24.1 ms | 624 MB | 0.748 / 0.536 |
| 1025² | 2,097,152 | 1,400,508 | 45.8 ms | 34.0 ms | 763 MB | 0.748 / 0.479 |

This is a synthetic random-latent **layer scaling** workload, not the image-trained encoder timing above. Query count remains 262,144 even at 1,050,625 fine control vertices. The times include both factor decoders, exact dynamic composition, one final image resampling, loss and VJP; they exclude one-time query-table setup. RSS includes PyTorch/runtime overhead and is sampled after each repeat, so it may miss a transient peak. The existing single A2+ same-host 1025² benchmark was about 40.8 ms forward including image warp/loss, 26.3 ms VJP and 708 MB sampled RSS, but it used a different random latent draw and five repeats; the comparison is indicative rather than a matched speed ratio. The full per-run records are `raw_results/cf2_scaling_{257,513,1025}_element_cpu.json`.
