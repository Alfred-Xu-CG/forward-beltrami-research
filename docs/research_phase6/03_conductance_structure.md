...
This rank result explains the measured Woodbury expression limit: the fast experiment altered only 16, 64, or 256 isolated selected edges out of 196,094 learnable active edges on the \(257^2\) grid. It cannot stand in for a smooth coarse-to-fine change to every conductance. Choosing structured cutsets instead of isolated diagonal edges is an open design experiment; the present result does not rule out useful low-rank families.
