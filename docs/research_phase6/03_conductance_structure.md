...
## 10. Why a coarse latent is not necessarily a low-rank matrix update

Let \(V\) contain all fine-grid vertices, including fixed boundary vertices, and orient each active mesh edge arbitrarily. Its incidence column \(u_e\in\mathbb R^{|V|}\) has \(+1\) at one endpoint, \(-1\) at the other and zero elsewhere. The full conductance Laplacian is \(L(c)=\sum_e c_eu_eu_e^\mathsf T\); the Dirichlet matrix \(A(c)\) is its interior-interior principal block. Suppose a single coarse latent \(t\) raises *many* conductances by \(\delta_e(t)>0\). If \(U\) collects their incidence columns and \(D=\operatorname{diag}(\delta_e)\), then \(\Delta L=UDU^\mathsf T\) and \(\Delta A=U_I D U_I^\mathsf T\), where \(U_I\) retains only interior rows. Since \(D\) is strictly positive, \(\operatorname{rank}(\Delta A)=\operatorname{rank}(U_I)\): the number of latent scalars controlling \(D\) does **not** enter this identity.

The incidence-rank formula makes that rank geometric. For each connected component of the graph formed by changed edges, let \(n_I\) be its number of interior vertices. If the component touches a fixed boundary vertex, its contribution to \(\operatorname{rank}(U_I)\) is \(n_I\); if it has no boundary vertex, its contribution is \(n_I-1\). To see this, a vector in \(\ker U_I^\mathsf T\) is constant along every changed edge; a touched fixed boundary forces that constant to zero, whereas an entirely interior component has one free constant. Thus a spatially distributed positive update can have rank near the full number of interior vertices even when it is controlled by one coarse coefficient. On a 17² triangular control grid, independent incidence construction gave: one changed edge rank 1; 16 disjoint cut edges rank 16; 56 edges covering one connected 5×5 interior-vertex patch rank 24; and all 734 active edges (conceivably controlled by just **one** scale parameter) rank 225, the full interior dimension. Numerical matrix rank agreed with the component formula in every case; see `raw_results/positive_update_rank17.json`. Scaling **every** conductance uniformly is a degenerate deformation example: its Dirichlet matrix update has full rank, but both matrix and right-hand side scale and the harmonic map itself is unchanged. The rank conclusion still applies to nonuniform positive spatial updates that actually change the map.

There is a sharper rank-one fact under the *positive-increment Laplacian* hypothesis. If \(\Delta L=uu^\mathsf T\) is nonzero, has zero row sums, and has nonpositive off-diagonals, then for all distinct \(i,j\), \(u_i u_j\le0\). Therefore \(u\) has at most one positive and at most one negative component; zero row sum forces one of each with equal magnitude. Hence \(u\) is a multiple of one edge incidence \(e_i-e_j\). A rank-one positive graph-Laplacian update can only raise one edge conductance. This does **not** prohibit lower-rank *indefinite* updates that raise some edges and lower others while final weights stay positive, nor does it prohibit non-Woodbury exact structure such as the implemented Schur blocks. It explains why a low-dimensional coarse latent is not, by itself, a route to a small exact Woodbury correction for dense spatial conductance variation.

## 11. Can unrestricted positive conductances represent the target at all?

The speed and optimization shortcomings above should not be confused with an intrinsic impossibility of the *all-edge* positive-conductance family. For a proposed sampled target map \(Y:V\to\mathbb R^2\), form a sparse matrix \(M(Y)\) with one column per active edge and two rows per interior vertex. For edge \(e=\{i,j\}\), its column has the two-vector \(Y_i-Y_j\) at interior endpoint \(i\) and \(Y_j-Y_i\) at interior endpoint \(j\), omitting rows for fixed boundary endpoints. The exact symmetric equilibrium condition is simply

\[
M(Y)c=0,\qquad c_{\min}\le c_e\le c_{\max}\quad\text{for every active edge }e.
\]

This is a linear feasibility problem **in conductances for a fixed target**, not a practical forward neural-layer solve. With positive \(c\), the corresponding Dirichlet matrix is SPD. Therefore, if the target has the prescribed boundary coordinates and satisfies these equations, it is the unique full-system equilibrium map; an independent sparse direct re-solve verifies the construction numerically. The ratio \(c_{\max}/c_{\min}\), not overall scale, is material because multiplying all conductances by one constant leaves the map unchanged. An infeasible bounded interval excludes only that interval; it does not prove infeasibility for all positive weights.

SciPy HiGHS and an independently assembled sparse Dirichlet solve gave:

| Analytic sampled target | Control vertices | Active edges | Allowed conductances | Feasibility status | Max direct-recovery coordinate error |
|---|---:|---:|---:|---|---:|
| base | 17² | 734 | [1,8] | feasible | \(1.58\times10^{-15}\) |
| base | 33² | 3,006 | [1,2] | reported infeasible | — |
| base | 33² | 3,006 | [1,4] | feasible | \(1.20\times10^{-14}\) |
| high32 | 97² | 27,454 | [1,16] | feasible | \(2.62\times10^{-13}\) |

For the feasible high32 case, the weighted equilibrium's largest absolute component residual was \(2.77\times10^{-12}\). The 97² sample resolves the 32-cycle term at three grid intervals per wavelength; it is **not** a 257² feasibility result. This establishes that a genuine high-frequency deformation can be represented by symmetric positive conductances on a moderately dense triangular grid, while the restricted interface family in §9 cannot exactly represent it. It does not tell us how cheaply to infer such weights from images or solve the full 257² system. The numerical infeasibility status for ratio two is not a separately extracted dual certificate. Reproduction code is `tools/phase6_conductance_inverse_feasibility.py`, with case data in `raw_results/conductance_inverse_*`.

An attempted 257² high32 feasibility LP had 196,094 edge variables and 130,050 equality rows; HiGHS reached its declared 300-second limit without a feasible point or infeasibility certificate. Its status is **unknown**, not “infeasible.” The attempt is recorded as `raw_results/conductance_inverse_high32_257_ratio16_timeout.json`. Better primal construction or a faster bounded least-squares feasibility method may settle this scale; the timeout is not a reason to abandon the all-edge route.

## 12. Optional symmetric sparse direct backend at 257²

The full all-edge positive-conductance layer need not use the block-Schur prototype. On the same 257² grid, we assembled its exact symmetric positive-definite interior Dirichlet matrix \(A(c)\in\mathbb R^{65025\times65025}\) and two-column boundary right-hand side. Three deterministic logit fields differed by a scalar offset while preserving the same sparsity pattern. For each, a fresh SciPy SuperLU factorization and an Intel oneMKL PARDISO factorization through the optional PyPardiso wrapper solved **the same** \(A(c)Y_I=b(c)\); the same factor then solved an independent two-column adjoint right-hand side. PARDISO used its symmetric-positive-definite mode (mtype=2) and the upper triangle of \(A\). Four CPU threads and float64 were used on Element; these are solver-only timings, not encoder plus image query timings.

Across three samples, the median fresh-factorization time was 0.150 s for PARDISO versus 0.433 s for SuperLU. The median PARDISO forward and adjoint substitutions were approximately 0.019 and 0.018 s; the corresponding SuperLU substitutions were about 0.019 and 0.018 s. Conductance matrix assembly took roughly 0.015–0.020 s per sample. Maximum coordinate discrepancy between the two solutions was below \(4\times10^{-14}\), full relative residual around \(5\times10^{-15}\), adjoint relative residual around \(2\times10^{-15}\), and all original fine-grid faces had positive signed areas (minimum normalized area about 0.64). Sampled process RSS was 688 MB. PARDISO's unsymmetric mode was slower than its symmetric mode (median factorization 0.177 s), so the SPD structure matters computationally.

This is a legitimate all-edge **exact** solver improvement, not Woodbury or approximate multigrid. It does not solve the main latency gap: refactorization after each unrestricted conductance change still costs roughly 0.15 s, much more than A4's tens-of-milliseconds complete image forward/VJP. The optional wrapper's public factorize method runs analysis and numerical factorization together on each changed matrix; we have **not** established symbolic-pattern reuse. Its storage conventions and solve correctness were checked at 65² before the 257² experiment. SuperLU factor times include conversion to CSC, whereas PARDISO input upper-triangle construction was timed outside its factor stage; the latter took part of assembly and is small relative to the approximately 0.28 s factorization gap, but comparisons should still be read as observed backend timings rather than an intrinsic asymptotic result. Full per-sample measurements are in raw_results/c_all_edge_pardiso_{spd,unsym}257_element_cpu.json, and the reproducer is tools/phase6_benchmark_pardiso_all_edge.py.

## 13. Exact PARDISO backend in a differentiable layer

The new optional PardisoAllEdgeTutteLayer uses precisely the same positive conductance vector, full fine-grid Dirichlet assembly, fixed boundary, and all-edge trainable image encoder as the block-Schur reference. Its input is a CPU float64 logit vector \(z\in\mathbb R^E\), \(E=196094\) at 257²; the conductance on edge \(e\) is \(c_e=c_{\min}+\log(1+\exp z_e)>0\). The output consists of the two coordinates of all 66,049 original-grid vertices; the represented map is their P1 extension on all 131,072 original triangles. For interior vertices \(I\), the two coordinate columns solve \(A(c)Y_I=b(c)\). One symmetric factorization is saved through the forward-to-backward interval, then solves \(A(c)\lambda=(\partial L/\partial Y)_I\). The exact first-order edge-logit VJP is

\[
\frac{\partial L}{\partial z_e}
=-\sigma(z_e)\,(Y_a-Y_b)\cdot(\lambda_a-\lambda_b),
\]

where \(a,b\) are edge endpoints, \(\sigma\) is the logistic derivative of softplus, and \(\lambda=0\) at fixed boundary vertices. This includes interior-boundary edges; omitting them would be an incorrect boundary gradient. The native factor is released after backward or no-gradient inference. A 17²/33² independently factored complete-system comparison and directional finite differences passed; the 257² image training additionally confirmed nonzero coarse and fine encoder-head gradients and positive represented face areas.

Under the **same one-pair** high32 image-to-latent protocol, encoder initialization, 257² controls, 512² image, 300 Adam steps, and Element four-thread CPU as the earlier all-edge Schur run, the PARDISO-backed layer gave image MSE 0.00177737 versus Schur 0.00177827, query-map RMSE 0.00460294 versus 0.00460386, face-centroid Beltrami RMSE 0.159395 versus 0.159364, and minimum signed area ratio 0.080996 versus 0.081058. The differences are numerical/optimization noise, not a material geometric improvement. Median logged forward times after the first step were 0.226 versus 0.574 s (about 2.5-fold faster); median backward times were 0.048 versus 0.047 s. Maximum observed process RSS was 881 versus 1688 MB. Both runs were on the same host and task, but RSS is a sampled process measurement and may reflect backend allocation policies. The PARDISO solve residual at the end was about \(5\times10^{-15}\). This makes the all-edge C candidate notably less expensive, yet it remains substantially slower than A4 and does not solve the observed image-to-latent geometric error.

Implementation and tests are in src/qcopt/neural_bijection/dense/pardiso_all_edge.py and tests/test_phase6_pardiso_all_edge.py. Reproduction: tools/phase6_train_conductance_image_to_latent.py with solver=pardiso; all 300-step records are in raw_results/trainpair_C_high32_pardiso257_512_element_cpu300.json. PyPardiso is optional and not required for the rest of the repository. No symbolic-factor reuse or all-edge Woodbury speedup is claimed.
