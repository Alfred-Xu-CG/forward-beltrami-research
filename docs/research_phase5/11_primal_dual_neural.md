...
2. A positive-conductance student maps a learnable face latent to a symmetric Tutte system. Strictly positive conductances and a convex boundary give the implemented graph family a hard discrete topology guarantee. The price is approximation: a fixed finite direction cone cannot reproduce every local Beltrami conductivity tensor exactly, while global operator and map errors require separate measurement.
...
For a solved scalar field u, the physical face flux is q_T = A_T grad u_T. The compatible algebraic object used by the implementation is instead the variational edge cochain

    q_var = H_A B0 u.

Interior weak balance is B0_I^T q_var = 0. For each interior primal edge, orient the dual edge from its right incident face to its left incident face and let D be the resulting face-to-interior-edge incidence matrix. The diagnostic solves

    D psi = (q_var restricted to interior edges),

where psi assigns one scalar to each face and one face value fixes the additive gauge. On a simply connected disk the weakly balanced cochain is dual exact under the stated incidence and rank hypotheses. The piecewise-constant physical vectors q_T generally have normal jumps and are not the object integrated by this equation.
...
For the zero-floor closed nonnegative cone of the explicit center-split and stellar direction sets, the largest worst-orientation uniform exact radius in Beltrami modulus is

    r_star = sqrt(2) - 1.

At the threshold some coefficients can be zero. Strict positivity therefore gives the open condition r < r_star, and a fixed positive floor introduces an additional approximation bias even inside the closed cone. The standard southwest-to-northeast triangulation has zero uniform radius because its missing projective direction makes some infinitesimal signed off-diagonal perturbations leave the positive cone. These are local tensor-cone statements, not global operator or map statements.
...
- No exact arbitrary prescribed-Beltrami reproduction is established. The finite-cone theorem obstructs exact local tensor representation by the fixed scalar directions; shared-edge operator accuracy and decoded-map accuracy are different questions and are measured separately rather than declared impossible by that theorem.
