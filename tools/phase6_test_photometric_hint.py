...
from qcopt.neural_bijection.dense import HierarchicalConvexQuadLocalLayer, evaluate_structured_p1_with_jacobian, local_photometric_logits
...
def main() -> None:
    ...
    train = make_dataset(args.train_count, image_side, 55101, return_coefficients=True, target_family="high32")
    test = make_dataset(args.test_count, image_side, 99317, return_coefficients=True, target_family="high32")
    line = torch.linspace(0.0, 1.0, side)
    ...
    source_vertices = torch.tensor(mesh.vertices.copy(), dtype=torch.float32)
    face_centroids = source_vertices[torch.tensor(mesh.faces.copy(), dtype=torch.long)].mean(dim=1)

    @torch.no_grad()
    def evaluate(
        dataset, window: int, ridge: float, gain: float, passes: int = 1, face_geometry: bool = False
    ) -> dict[str, object]:
        image_losses = []
        map_losses = []
        area = float("inf")
        ...
        for start in range(0, len(fixed_all), args.batch):
            ...
            if face_geometry:
                ...
                predicted_mu, target_mu, sampled_mu = (
                    _mu(jacobian) for jacobian in (predicted_jacobian, target_jacobian, sampled_jacobian)
                )
        ...
        for ridge in ridges:
            for gain in gains:
                for passes in range(1, args.max_passes + 1):
                    measured = evaluate(train, window, ridge, gain, passes=passes)
                    candidates.append({"window": window, "ridge": ridge, "gain": gain, "passes": passes, "train": {
                        key: value for key, value in measured.items() if key != "projected_fine_amplitudes"
                    }})
    ...
    heldout = evaluate(
        test, chosen["window"], chosen["ridge"], chosen["gain"], passes=chosen["passes"], face_geometry=True
    )
    print(json.dumps({
